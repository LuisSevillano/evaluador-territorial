...
<aside class="inspector">
	<section class="panel">
		...
		{#if selectedMunicipio}
			...
			<div class="metric-grid">
				...
				<div class={`metric ${accessTone(selectedMunicipio.travel_bucket)}`}><span>Accesibilidad</span><strong>{selectedMunicipio.travel_bucket}</strong></div>
				<div class={`metric ${climateTone(selectedMunicipio.precip_annual_mm)}`}><span>Precipitacion</span><strong>{selectedMunicipio.precip_annual_mm} mm</strong></div>
				<div class="metric"><span>Invierno / Verano</span><strong>{selectedMunicipio.temp_winter_mean_c} / {selectedMunicipio.temp_summer_mean_c} C</strong></div>
				<div class="metric"><span>Enero / Julio</span><strong>{selectedMunicipio.temp_jan_mean_c} / {selectedMunicipio.temp_jul_mean_c} C</strong></div>
				<div class="metric"><span>% forestal / agua</span><strong>{selectedMunicipio.forest_pct ?? '-'} / {selectedMunicipio.water_pct ?? '-'}</strong></div>
			</div>
			<div class="charts">
				<div class="chart-card">
					<p>Temperatura mensual (media + rango)</p>
					<ClimateTempLineChart data={climateSeries} provinceData={provinceClimateSeries} ccaaData={ccaaClimateSeries} />
					{#if context?.tempAmplitude !== null}
						<small>Amplitud anual: {context?.tempAmplitude?.toFixed(1)} C (mas alto = estacionalidad mas marcada).</small>
					{/if}
				</div>
				<div class="chart-card">
					...
					{#if context?.wettest && context?.driest}
						<small>Pico humedo mes {context.wettest.month} ({context.wettest.precip_mm.toFixed(0)} mm) · valle seco mes {context.driest.month} ({context.driest.precip_mm.toFixed(0)} mm).</small>
					{/if}
				</div>
			</div>
	...
	<section class="panel score-panel">
		...
		<div class="control score-control">
			...
			<input id="rw-climate" name="rw-climate" type="range" min="0" max="100" step="1" value={weightsRaw.climateWeight} oninput={(e) => onClimateWeightChange(toNumber(e))} />
		</div>
		<div class="control score-control">
			<label for="rw-access">Peso accesibilidad: {weightsRaw.accessWeight}</label>
			<input id="rw-access" name="rw-access" type="range" min="0" max="100" step="1" value={weightsRaw.accessWeight} oninput={(e) => onAccessWeightChange(toNumber(e))} />
		</div>
		<div class="control score-control">
			<label for="rw-nature">Peso naturaleza: {weightsRaw.natureWeight}</label>
			<input id="rw-nature" name="rw-nature" type="range" min="0" max="100" step="1" value={weightsRaw.natureWeight} oninput={(e) => onNatureWeightChange(toNumber(e))} />
		</div>
...
<style>
	.inspector { width: 100%; max-width: 360px; height: 100%; min-height: 0; overflow-y: auto; background: linear-gradient(180deg, rgba(246, 239, 227, 0.9), rgba(234, 222, 198, 0.92)); border-left: 1px solid rgba(21, 32, 33, 0.18); padding: 0.9rem; display: grid; gap: 0.8rem; box-sizing: border-box; }
	.panel { border: 1px solid rgba(21, 32, 33, 0.16); border-radius: 12px; padding: 0.95rem; background: rgba(255, 255, 255, 0.72); }
	h2,h3,p { margin: 0; }
	h2 { font-family: 'Fraunces', serif; font-size: 1rem; margin-bottom: 0.5rem; }
	h3 { font-family: 'Fraunces', serif; font-size: 1.1rem; }
	p { font-size: 0.82rem; line-height: 1.35; color: #3f5652; }
	.title-row + p { margin-top: 0.12rem; font-size: 0.78rem; }
	.title-row { display: flex; align-items: center; justify-content: space-between; gap: 0.4rem; }
	.ghost-btn { width: auto; border: 1px solid rgba(19, 63, 70, 0.24); border-radius: 999px; background: rgba(255, 255, 255, 0.78); padding: 0.2rem 0.5rem; font-size: 0.74rem; }
	.metric-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.6rem; margin-top: 0.7rem; }
	.metric-grid div { padding: 0.55rem; border-radius: 10px; background: rgba(238, 248, 245, 0.64); border: 1px solid rgba(19, 63, 70, 0.16); }
	.metric.score { border-color: rgba(35, 98, 108, 0.45); background: rgba(224, 244, 247, 0.72); }
	.metric.good { border-color: rgba(15, 118, 110, 0.42); background: rgba(220, 248, 241, 0.72); }
	.metric.mid { border-color: rgba(180, 111, 36, 0.38); background: rgba(252, 242, 222, 0.74); }
	.metric.bad { border-color: rgba(170, 45, 45, 0.38); background: rgba(252, 234, 234, 0.74); }
	...
	.charts { display: grid; gap: 0.7rem; margin-top: 0.75rem; }
	.chart-card { border: 1px solid rgba(19, 63, 70, 0.16); border-radius: 10px; background: rgba(255, 255, 255, 0.68); padding: 0.55rem; }
	.chart-card p { font-size: 0.72rem; letter-spacing: 0.03em; text-transform: uppercase; color: #3d5552; margin-bottom: 0.22rem; }
	.chart-card small { display: block; margin-top: 0.25rem; font-size: 0.7rem; color: #3b5250; }
	.shortlist-btn { margin-top: 0.6rem; cursor: pointer; border: 1px solid rgba(21, 32, 33, 0.22); border-radius: 999px; background: linear-gradient(120deg, #2f7d85, #245f66); color: #f7f4ec; padding: 0.45rem 0.72rem; font-size: 0.78rem; transition: transform 160ms ease, filter 160ms ease; }
	.shortlist-btn:hover { transform: translateY(-1px); filter: brightness(1.05); }
	.shortlist-btn:focus-visible { outline: 2px solid #2f7d85; outline-offset: 2px; }
	.chips-wrap { display: grid; gap: 0.35rem; }
	.preset-wrap { margin-top: 0.35rem; }
	.inspector :global(.chips-wrap .chip-btn) { width: 100%; }
	.control { display: grid; gap: 0.3rem; margin-top: 0.55rem; }
	.control label { font-size: 0.76rem; letter-spacing: 0.02em; color: #3f5853; }
	.score-control { max-width: 230px; }
	.score-control input[type='range'] { height: 10px; }
	.muted { color: #48615d; font-size: 0.76rem; margin-top: 0.4rem; line-height: 1.35; }
