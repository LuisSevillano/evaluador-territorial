# Data dictionary completo (`municipios_v2`)

Está tabla documenta todas las columnas exportadas en `output/municipios_v2.csv` y `frontend/static/data/municipios_v2.json`.

| Campo | Tipo | Unidad/rango | Descripcion | Origen |
|---|---|---|---|---|
| `id` | string | codigo INE | Identificador principal para frontend. | `05_export_frontend_v2.R` |
| `codigo` | string | INE | Codigo municipal. | base municipal |
...
| `travel_bucket` | string | buckets | Bucket de accesibilidad principal. | `03_isochrones.R` |
...
| `transporte_norm` | number | [0,1] | Accesibilidad transporte general normalizada. | `04_transporte_distance.R` |
...
| `renfe_tipo_servicio` | string | categoria | Tipo de servicio ferroviarío. | `04b_transporte_renfe.R` |
...
| `river_access_score` | number | 0-100 | Score de acceso fluvial recreativo. | `04g_banio_score_simple.R` |
...
| `river_method_versión` | string | versión | Versión del metodo fluvial. | `04g_banio_score_simple.R` |
...
| `mixed_score` | number | [0,1] | Score mixto final del atlas. | `05_export_frontend_v2.R` |

## Notas de interpretacion

- `mixed_score` no es causal ni prescriptivo: es comparativo multicriterio.
- `river_access_score` mide acceso potencial recreativo a río, no calidad de agua.
